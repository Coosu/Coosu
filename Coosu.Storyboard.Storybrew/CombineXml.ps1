...
try {
    ...
    Select-Xml -Path "$($baseDir)\Coosu.Storyboard.Advanced.xml" -XPath '/doc/members/*' | ForEach-Object { $members.Add([System.Xml.Linq.XElement]::Parse($_.Node.OuterXml)) }

    $str = "<?xml version=""1.0""?>`n$($xdoc.ToString())";
    Write-Output $str >"$($baseDir)\Coosu.Storyboard.Storybrew-ILMerged$($append).xml"
    Write-Output "Wrote new document to ""$($baseDir)\Coosu.Storyboard.Storybrew-ILMerged$($append).xml"""
}
catch {
