...
namespace Backports.System
{
    internal static partial class Number
    {
        ...
        // 29 for the longest input + 1 for rounding
        internal const int DecimalNumberBufferLength = 29 + 1 + 1;

        // 767 for the longest input + 1 for rounding: 4.9406564584124654E-324
        internal const int DoubleNumberBufferLength = 767 + 1 + 1;

        // 10 for the longest input: 2,147,483,647
        internal const int Int32NumberBufferLength = 10 + 1;

        // 19 for the longest input: 9,223,372,036,854,775,807s
        internal const int Int64NumberBufferLength = 19 + 1;

        // 112 for the longest input + 1 for rounding: 1.40129846E-45
        internal const int SingleNumberBufferLength = 112 + 1 + 1;

        // 19 for the longest input + 1 for rounding (+1 for the null terminator)
        internal const int HalfNumberBufferLength = 21;

        // 10 for the longest input: 4,294,967,295
        internal const int UInt32NumberBufferLength = 10 + 1;

        // 20 for the longest input: 18,446,744,073,709,551,615
        internal const int UInt64NumberBufferLength = 20 + 1;
...
//            }
...
#if DEBUG
                Debug.Assert(
                    Kind == NumberBufferKind.Integer || Kind == NumberBufferKind.Decimal
            ...
            public          NumberBufferKind   Kind   { get; }

            public ref byte GetRefMut() => ref _digits[0];
