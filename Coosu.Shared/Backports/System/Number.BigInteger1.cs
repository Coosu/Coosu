...
namespace Backports.System
{
    internal static partial class Number
    {
        ...
        internal struct BigInteger
        {
            ...
            // (3658 + (sizeof(int) * 8 - 1)) / (sizeof(int) * 8) = 115
            ...
            public static int Compare(in BigInteger lhs, in BigInteger rhs)
            {
                ...
                var lengthDelta = lhsLength - rhsLength;
            ...
            public static void DivRem(in BigInteger lhs, in BigInteger rhs, out BigInteger quo, out BigInteger rem)
            {
                ...
                {
                    //var quotient = MathP.DivRem(lhs._blocks[0], rhs._blocks[0], out var remainder);
                    var quotient = MathP.DivRem(At(in lhs, 0), At(in rhs, 0), out var remainder);
                ...
                if (rhsLength == 1)
                {
                    ...
                    {
                        var value = (carry << 32) | At(in lhs, i);//lhs._blocks[i];
                        var digit = MathP.DivRem(value, rhsValue, out carry);

                        if (digit == 0 && i == quoLength - 1)
                            quoLength--;
                        else
                            //quo._blocks[i] = (uint) digit;
                            AtMut(ref quo, i) = (uint)digit;
                    }
                ...
                else
                {
                    ...
                    //var divHi = rhs._blocks[rhsLength - 1];
                    var divHi = At(in rhs, rhsLength - 1);
                    //var divLo = rhs._blocks[rhsLength - 2];
                    var divLo = At(in rhs, rhsLength - 2);

                    // We measure the leading zeros of the divisor
                    var shiftLeft = BitOperations.LeadingZeroCount(divHi);
                    var shiftRight = 32 - shiftLeft;
                    ...
                    if (shiftLeft > 0)
                    {
                        ...
                        if (rhsLength > 2)
                            ...
                            divLo |= At(in rhs, rhsLength - 3) >> shiftRight;
                    }

                    // Then, we divide all of the bits as we would do it using
                    // pen and paper: guessing the next digit, subtracting, ...
                    for (var i = lhsLength; i >= rhsLength; i--)
                    {
                        var n = i - rhsLength;
                        //var t = i < lhsLength ? rem._blocks[i] : 0;
                        var t = i < lhsLength ? At(in rem, i) : 0;

                        //var valHi = ((ulong)t << 32) | rem._blocks[i - 1];
                        var valHi = ((ulong)t << 32) | At(in rem, i - 1);
                        //var valLo = i > 1 ? rem._blocks[i - 2] : 0;
                        var valLo = i > 1 ? At(in rem, i - 2) : 0;
                        ...
                        if (shiftLeft > 0)
                        {
                            ...
                            if (i > 2)
                                ...
                                valLo |= At(in rem, i - 3) >> shiftRight;
                        }
            ...
            public static uint HeuristicDivide(ref BigInteger dividend, in BigInteger divisor)
            {
                ...
                var quotient = At(in dividend, lastIndex) / (At(in divisor, lastIndex) + 1);
                ...
                if (quotient != 0)
                {
                    ...
                    do
                    {
                        ...
                        var product = (ulong)At(in divisor, index) * quotient + carry;
                        carry = product >> 32;

                        //var difference = (ulong)dividend._blocks[index] - (uint)product - borrow;
                        var difference = (ulong)At(in dividend, index) - (uint)product - borrow;
                        borrow = (difference >> 32) & 1;

                        //dividend._blocks[index] = (uint)difference;
                        AtMut(ref dividend, index) = (uint)difference;
                    ...
                    while (divisorLength > 0 && At(in dividend, divisorLength - 1) == 0)
                        divisorLength--;
                ...
                if (Compare(in dividend, in divisor) >= 0)
                {
                    ...
                    do
                    {
                        ...
                        var difference = (ulong)At(in dividend, index) - At(in divisor, index) - borrow;
                        borrow = (difference >> 32) & 1;
                    ...
                    // Remove all leading zero blocks from dividend
                    //while (divisorLength > 0 && dividend._blocks[divisorLength - 1] == 0) 
                    while (divisorLength > 0 && At(in divisor, divisorLength - 1) == 0)
                        divisorLength--;
            ...
            public static void Multiply(in BigInteger lhs, uint value, out BigInteger result)
            {
                ...
                {
                    //var product = (ulong)lhs._blocks[index] * value + carry;
                    var product = (ulong)At(in lhs, index) * value + carry;
                    //result._blocks[index] = (uint)product;
                    AtMut(ref result, index) = (uint)product;
            ...
            public static void Multiply(in BigInteger lhs, in BigInteger rhs, out BigInteger result)
            {
                ...
                ref readonly var large = ref lhs;
                var largeLength = lhs._length;

                ref readonly var small = ref rhs;
                var smallLength = rhs._length;
                ...
                var maxResultLength = smallLength + largeLength;
            ...
            {
                var blocksToShift = DivRem32(exponent, out var remainingBitsToShift);
                result = default;
            ...
            public static void Pow10(uint exponent, out BigInteger result)
            {
                ...
                // Validate that `s_Pow10BigNumTable` has exactly enough trailing elements to fill a BigInteger1 (which contains MaxBlockCount + 1 elements)
                ...
                SetUInt32(out var temp1, s_Pow10UInt32Table[exponent & 0x7]);
                ref var lhs = ref temp1;

                SetZero(out var temp2);
                ref var product = ref temp2;
                ...
                while (exponent != 0)
                {
                    ...
                    if ((exponent & 1) != 0)
                    {
                        ...
                        //{
                        //    ref var rhs = ref *(BigInteger1*)pBigNumEntry;
            ...
            public void MultiplyPow10(uint exponent)
            {
                ...
                {
                    Pow10(exponent, out var poweredValue);
            ...
            public void ShiftLeft(uint shift)
            {
                ...
                var blocksToShift = DivRem32(shift, out var remainingBitsToShift);

                // Copy blocks from high to low
                var readIndex = length - 1;
                var writeIndex = readIndex + (int)blocksToShift;
