...
public static class PathApproximator
{
    ...
    public static Vector2[] BSplineToBezier(ReadOnlySpan<Vector2> controlPoints, int degree)
    {
        ...
        // does not support.
        throwIfLessThan(degree, 1, nameof(degree));
    ...
    {
        throwIfLessThan(numControlPoints, 2, nameof(numControlPoints));

        throwIfLessThan(numTestPoints, 2, nameof(numTestPoints));
    ...
    private static void throwIfLessThan(int value, int other, string paramName)
    {
#if NETSTANDARD2_0
        if (value < other) throw new ArgumentOutOfRangeException(paramName, value, $"{paramName} ({value}) must be greater than or equal to {other}.");
#else
    ...
    private static class TensorPrimitives
    {
        public static void Multiply(ReadOnlySpan<float> x, float y, Span<float> destination)
        {
            for (int i = 0; i < x.Length; i++) destination[i] = x[i] * y;
        }
        ...
        public static void MultiplyAdd(ReadOnlySpan<float> x, float y, ReadOnlySpan<float> z, Span<float> destination)
        {
            for (int i = 0; i < x.Length; i++) destination[i] = x[i] * y + z[i];
        }
